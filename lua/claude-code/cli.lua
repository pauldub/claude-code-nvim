...
local function validate_config(cfg)
  ...
  valid = valid
    and validate_option('output_format', cfg.output_format, { 'text', 'json', 'stream-json' }, 'output_format must be one of: text, json, stream-json')
...
function M.setup(opts, global_config)
  ...
  if not validate_config(config) then
    vim.schedule(function()
      vim.notify('Claude Code CLI configuration has errors. Some features may not work correctly.', vim.log.levels.WARN)
    end)
  end
      ...
      on_exit = function(j, return_val)
        ...
        vim.schedule(function()
          ...
          if return_val ~= 0 then
            vim.notify("Claude Code CLI not found. Make sure it's installed and in your PATH.", vim.log.levels.WARN)
            config.is_available = false
          else
            ...
            if version then
              ...
              if compare_versions(version, config.min_version) < 0 then
                vim.notify(string.format('Claude Code CLI version %s is below required minimum %s', version, config.min_version), vim.log.levels.WARN)
                config.is_available = false
              else
  ...
  if file then
    ...
    if config.debug_mode and prompt_content and prompt_content ~= "stdin" then
      if #prompt_content > 500 then
        prompt_content = prompt_content:sub(1, 500) .. "... [truncated, total length: " .. #prompt_content .. "]"
      end
          ...
          if config.log_commands then
            ...
            if file then
              ...
              if #log_response > 1000 then
                log_response = log_response:sub(1, 1000) .. "... [truncated, total length: " .. #result .. "]"
              end
            ...
            if status then
              ...
              if parsed.result then
                ...
                if parsed.cost_usd then
                  vim.notify(string.format('Claude request cost: $%.6f, time: %dms', parsed.cost_usd, parsed.duration_ms or 0), vim.log.levels.INFO)
                end
              end
            else
